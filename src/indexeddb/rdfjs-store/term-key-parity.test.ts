// Term-key parity: @worlds/indexeddb's row keys must never diverge from the
...
import { termKey as localTermKey } from "@/indexeddb/term/term-key.ts";
...
Deno.test(
  "term-key parity — @worlds/indexeddb matches @wazoo/sparql-engine on the term corpus",
  () => {
